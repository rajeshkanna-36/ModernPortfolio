        ...
        {[...layers].reverse().map((layer, reversedIndex) => {
          ...
          return (
            ...
            >
              ...
              {/* === BUILDO PROJECT DOODLES (Construction) === */}
              <AnimatePresence>
                {/* 1. Hammer (Right Card) */}
                {originalIndex === 1 && title === "Buildo" && isHovered && (
                  <motion.div 
                    initial={{ opacity: 0, y: 30, rotate: 45, scale: 0.5 }}
                    animate={{ opacity: 1, y: -10, rotate: 15, scale: 1 }}
                    exit={{ opacity: 0, y: 20, scale: 0.5 }}
                    transition={{ type: "spring", stiffness: 400, damping: 15, delay: 0.1 }}
                    className="absolute -top-12 -right-10 z-[200] text-zinc-800 pointer-events-none drop-shadow-md"
                  >
                     <svg width="48" height="48" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" className="opacity-80">
                        <path d="M30 70 L70 30" />
                        <path d="M60 10 L90 40 L80 50 L50 20 Z" fill="currentColor" stroke="none" />
                     </svg>
                  </motion.div>
                )}
                {/* 2. Construction Cone (Left Card) */}
                {originalIndex === 3 && title === "Buildo" && isHovered && (
                  <motion.div 
                    initial={{ opacity: 0, x: 20, rotate: -20, scale: 0.5 }}
                    animate={{ opacity: 1, x: 0, rotate: -5, scale: 1 }}
                    exit={{ opacity: 0, x: 10, scale: 0.5 }}
                    transition={{ type: "spring", stiffness: 400, damping: 15, delay: 0.15 }}
                    className="absolute -top-8 -left-12 z-[200] pointer-events-none drop-shadow-md"
                  >
                     <svg width="48" height="48" viewBox="0 0 100 100" fill="none" strokeLinecap="round" strokeLinejoin="round" className="opacity-90">
                        <path d="M50 20 L80 80 L20 80 Z" fill="#FFE600" stroke="none" />
                        <path d="M35 50 L65 50" stroke="white" strokeWidth="12" />
                        <path d="M10 85 L90 85" strokeWidth="8" stroke="#FFE600" />
                     </svg>
                  </motion.div>
                )}
                {/* 3. Ruler / Measurement (Middle Card) */}
                {originalIndex === 2 && title === "Buildo" && isHovered && (
                  <motion.div 
                    initial={{ opacity: 0, y: 30, rotate: 10, scale: 0.5 }}
                    animate={{ opacity: 1, y: -25, rotate: -10, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.5 }}
                    transition={{ type: "spring", stiffness: 400, damping: 15, delay: 0.2 }}
                    className="absolute -top-16 left-1/4 z-[200] text-[#111111] pointer-events-none drop-shadow-md"
                  >
                     <svg width="40" height="40" viewBox="0 0 100 100" fill="#FFE600" stroke="currentColor" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" className="opacity-90">
                        <rect x="10" y="40" width="80" height="20" rx="2" />
                        <path d="M20 40 L20 50 M40 40 L40 50 M60 40 L60 50 M80 40 L80 50" />
                     </svg>
                  </motion.div>
                )}
              </AnimatePresence>
